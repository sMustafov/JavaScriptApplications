...
import CommentBookView from './Views/CommentBookView'
...
export default class App extends Component {
        ...
        function loadBooksSuccess(books) {
            ...
            this.showView(
                <BooksView
                    ...
                    commentBookClicked={this.showCreateCommentView.bind(this)}
                />
            );
        }
    }
    ...
    editBook(bookId, name, publisher, description, price) {
        KinveyRequester.editBook(bookId, name, publisher, description, price)
   ...
   /* confirmComment(bookId) {
        alert("Comment button clicked");
        KinveyRequester.findBookById(bookId)
            .then(loadBookForCommentSuccess.bind(this));

        function loadBookForCommentSuccess(bookInfo) {
            this.showView(
                <CommentBookView
                    onsubmit={this.showCreateCommentView.bind(this)}
                    bookId={bookInfo._id}
                    name={bookInfo.name}
                    publisher={bookInfo.publisher}
                    description={bookInfo.description}
    ...
    createBook(name, publisher, description, price) {
        KinveyRequester.createBook(name, publisher, description, price)
    ...
    showCreateCommentView(bookId) {
        ...
        function loadBookForCommentSuccess(bookInfo) {
            this.showView(
                <CommentBookView
                    onsubmit={this.commentBook.bind(this)}
                    bookId={bookInfo._id}
                    name={bookInfo.name}
                    publisher={bookInfo.publisher}
                    description={bookInfo.description}
      ...
      // this.showView(<CommentBookView onsubmit={this.commentBook.bind(this)} />);
    }

    commentBook(id, name, publisher, description, price, comment) {
       /* alert("ID " + id);
        alert("name: " + name);
        ...
        KinveyRequester.commentBook(id, name, publisher, description, price, comment)
            .then(createBookSuccess.bind(this));

        function createBookSuccess() {
            this.showCommentsView(id);
            this.showInfo("Book commented.");
        }
    }

    showCommentsView(id) {
        KinveyRequester.findCommentsById(id)
            .then(loadBooksSuccess.bind(this));
        alert(this.state.username)
        function loadBooksSuccess(books){
            //alert(books[0].commentId)
            this.showInfo("Book commented.");
            this.showView(
                <CommentsView
                    books={books}
